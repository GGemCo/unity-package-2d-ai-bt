...
namespace GGemCo2DAiBtEditor
{
    ...
    /// </summary>
    internal static class BtSkillDropdownProvider
    {
        ...
        /// <summary>
        /// skill 테이블을 파싱하여 드롭다운 옵션을 반환합니다.
        /// </summary>
        public static IReadOnlyList<SearchableDropdownUtility.Option<int>> GetOptions(bool forceReload = false)
        {
            ...
            _cachedHash = hash;
            _cached = ParseSkillTable(content);
            if (_cached.Count == 0)
                _cached.Add(new SearchableDropdownUtility.Option<int>("0", "(no skills)", 0));
        ...
        private static List<SearchableDropdownUtility.Option<int>> ParseSkillTable(string content)
        {
