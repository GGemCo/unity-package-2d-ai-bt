...
namespace GGemCo2DAiBtEditor
{
    public class SettingMonsterBt : DefaultAddressable
    {
        ...
        public void Setup(EditorSetupContext ctx = null)
        {
            ...
            Dictionary<int, StruckTableMonsterPhase> phaseDictionary = GGemCo2DCoreEditor.TableLoaderManager.LoadMonsterPhaseTable().GetDatas();
            ...
            if (group)
            {
                // monster/monster_phase 양쪽에서 BT 파일명을 수집합니다.
                var btFileNames = new HashSet<string>(StringComparer.Ordinal);
                ...
                foreach (KeyValuePair<int, StruckTableMonster> outerPair in dictionary)
                {
                    ...
                    if (info == null || info.Uid <= 0)
                        continue;
                    if (string.IsNullOrEmpty(info.BtFileName))
                        continue;

                    btFileNames.Add(info.BtFileName);
                }

                foreach (KeyValuePair<int, StruckTableMonsterPhase> phasePair in phaseDictionary)
                {
                    StruckTableMonsterPhase phaseInfo = phasePair.Value;
                    if (phaseInfo == null)
                        continue;
                    if (string.IsNullOrEmpty(phaseInfo.BtFileName))
                        continue;

                    btFileNames.Add(phaseInfo.BtFileName);
                }

                foreach (string btFileName in btFileNames)
                {
                    string key = $"{ConfigAddressableKeyAiBt.MonsterBt}_{btFileName}";
                    string assetPath = $"{ConfigAddressablePathAiBt.MonsterBt.Root}/{btFileName}.asset";
                    const string label = "";
